...
* **`StaticCollision`** — the concrete implementation of the `mercs2_core::physics_query::PhysicsQuery`
  seam, backed by a static world-space triangle set (buildings, roads, terrain mesh) plus an optional
...
* **`CharacterController`** — the on-foot locomotion state machine (`OnGround` / `Jumping` / `InAir`)
  ...
  `StaticCollision` today and against the full Havok world later.

The `broadphase` module is a separate, lighter-weight API: free functions over a raw `&[[Vec3; 3]]` with no
...
| triangle set + `Heightmap` | `hkpMoppBvTreeShape` + `hkpSampledHeightFieldShape` world |
| `broadphase` camera raycast | the exe's `CameraCollisionCastRay` radius² probe |
...
Library only — no binaries. Build a `StaticCollision` over world geometry, hand it to a
`CharacterController`, and step:

```rust
use mercs2_core::glam::Vec3;
use mercs2_core::physics_query::PhysicsQuery;
use mercs2_physics::{CharacterController, CharacterInput, Heightmap, RigidBody, StaticCollision};
...
// Or with terrain: StaticCollision::with_heightmap(tris, Heightmap::new(ox, oz, cell, w, d, heights))
...
The direct-tri API (what the engine's player + camera use) takes the triangle list itself:

```rust
use mercs2_core::glam::Vec3;
use mercs2_physics::broadphase;

let tris: Vec<[Vec3; 3]> = vec![/* world-space collision triangles */];
let feet = broadphase::move_character(&tris, Vec3::ZERO, Vec3::X * 0.1, 0.4, 1.8, 0.5, true);
let ground = broadphase::ground_below(&tris, feet, 0.4, 4.0);          // landing probe
let boom = broadphase::raycast(&tris, feet, Vec3::Z, 5.0);             // camera boom
```

## Modules

* **(crate root)** — `StaticCollision` (the `PhysicsQuery` impl: raycast / closest_point /
...
* **`broadphase`** — direct-triangle collision over `&[[Vec3; 3]]`, no world object: `ray_tri`,
...
* **No broadphase.** Every query is a linear scan over the triangle list with a cheap proximity cull.
  ...
  grow.
* **The two culls have different tuning.** The crate-root `StaticCollision` culls by distance to a
  triangle's *first vertex* — it is tuned for the game's **small** world triangles (the tests build
  tiled floors for exactly this reason). The `broadphase` module culls by triangle **bbox** and is therefore
...
* **`closest_point`'s sign is only meaningful for consistently-wound closed shells** (it is taken from
  the nearest triangle's wound normal). An open triangle set has no well-defined inside.
