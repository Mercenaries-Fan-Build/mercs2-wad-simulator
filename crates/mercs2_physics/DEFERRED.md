...
faithfulness blocker for the `PhysicsQuery` bridge (`StaticCollision`); each is tagged.

- **DONE (controller pass).** `move_character` is now a **swept linear cast** (conservative
  advancement, `StaticCollision::linear_cast` / `move_swept`) — tunnel-free at any speed — and a
  ...
  `RigidBody` + `StaticCollision::step_rigid_body` (semi-implicit Euler + impulse resolve) covers
...
- **Full rigid-body dynamics (props/debris).** `RigidBody` is a sphere with no rotation/inertia tensor,
  no body↔body contacts (only body↔static collision), and a single deepest-contact resolve. The retail
...
- **Broadphase (MOPP BV-tree).** Queries are a linear scan over the triangle set with a cheap sphere
...
- **Closest-point sign for arbitrary triangle set.** `closest_point`'s inside/outside sign is taken from the
  nearest triangle's wound normal, which is only meaningful for consistently-wound (outward-facing)
  closed shells. An open triangle set has no well-defined inside; the real `getClosestPoints` uses the shape's
