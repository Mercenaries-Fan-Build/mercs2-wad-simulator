...
fn explore_anim_class_counts() {
    ...
    for s in 0..3 {
        for k in 0..7 {
            secs[s][k] = u32le(buf, sh + s * 48 + 20 + k * 4);
        }
    }
    let body0 = sh + 3 * 48;
    let data_pk = body0 + secs[0][6] as usize + secs[1][6] as usize;
    eprintln!(
        "sh={sh} body0={body0} data_pk={data_pk} file_len={}",
        buf.len()
    );
    ...
    while k + 12 <= data_pk + d_end {
        ...
        if wavelet_src.is_none() && src != 0 {
            wavelet_src = Some(src as usize);
        }
        k += 12;
    }
    ...
    for row in 0..(200 / 16) {
        ...
        for c in 0..16 {
            ...
            if o < buf.len() {
                hex += &format!("{:02x} ", buf[o]);
                let ch = buf[o];
                asc.push(if ch >= 0x20 && ch < 0x7f {
                    ch as char
                } else {
                    '.'
                });
    ...
    if let Some(ws) = wavelet_src {
        ...
        eprintln!(
            "  +8  m_type(int)               = {}",
            u32le(buf, o + 8) as i32
        );
        eprintln!("  +12 m_duration(f32)            = {}", f32le(buf, o + 12));
        eprintln!(
            "  +16 m_numTransformTracks(int)  = {}",
            u32le(buf, o + 16) as i32
        );
        eprintln!(
            "  +20 m_numFloatTracks(int)      = {}",
            u32le(buf, o + 20) as i32
        );
