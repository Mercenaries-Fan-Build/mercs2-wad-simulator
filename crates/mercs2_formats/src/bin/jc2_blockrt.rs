...
fn main() {
    ...
    for bi in 0..arch.indx.len() {
        ...
        if (0..c)
            .any(|e| 4 + e * 16 + 16 <= d.len() && read_u32_le(&d, 4 + e * 16 + 4) == 0x5647_C35D)
        {
            blk = Some(d);
            break;
        }
    ...
    for ei in 0..count {
        expected += read_u32_le(&blk, 4 + ei * 16 + 12) as usize;
    }
    println!(
        "block len {} | header+entries+bodies = {} | trailing = {}",
        blk.len(),
        expected,
        blk.len() as i64 - expected as i64
    );
    ...
    println!(
        "wrote output/block3185_original.bin ({} bytes, verbatim)",
        blk.len()
    );
    if expected < blk.len() {
        let t = &blk[expected..];
        println!(
            "TRAILING {} bytes, first 32: {:02X?}",
            t.len(),
            &t[..32.min(t.len())]
        );
