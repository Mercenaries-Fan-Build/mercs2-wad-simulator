...
fn main() {
    ...
    println!(
        ...
        we.len(),
        std::str::from_utf8(&we[0..4]).unwrap_or("????")
    );
    ...
    let veh_hashes: Vec<(String, u32)> = veh_classes
        .iter()
        .map(|c| (c.to_string(), pandemic_hash_m2(c)))
        .collect();
    ...
    for g in &groups {
        let (Some(schema), Some(data)) = (g.schema(), g.data.as_ref()) else {
            continue;
        };
    ...
    for (n, k) in &bit31_examples {
        println!("    bit31 key 0x{k:08X} in {n}");
    }
        ...
        if let (Some(schema), Some(data)) = (g.schema(), g.data.as_ref()) {
            ...
            if let Some(h) = detect(data.len(), stride) {
                ...
                for rec in data[h..].chunks_exact(stride) {
                    ...
                    if hit_h || hit_m {
                        println!(
                            "    ModelName key=0x{key:08X} -> model=0x{model:08X}{}{}",
    ...
    for (cname, chash) in &veh_hashes {
        let Some(g) = groups
            .iter()
            .find(|g| g.type_hash == Some(*chash) || g.name.as_deref() == Some(cname.as_str()))
        else {
            continue;
        };
        ...
        let Some(data) = g.data.as_ref() else {
            continue;
        };
        if schema.is_variable_length() {
            println!(
                "\n### {cname}: variable-length (StringRef) — {} data bytes",
                data.len()
            );
            continue;
        }
        ...
        let mut bit31 = 0;
        for rec in data[h..].chunks_exact(stride) {
            if u32::from_le_bytes([rec[0], rec[1], rec[2], rec[3]]) & 0x8000_0000 != 0 {
                bit31 += 1;
            }
        }
    ...
    let targets: Vec<u32> = std::env::args()
        .skip(2)
    ...
    for th in &targets {
        ...
        for g in &groups {
            ...
            for rec in data[h..].chunks_exact(stride) {
                ...
                println!(
                    "  {:>26} (0x{:08X}) stride {stride}: {}",
                    g.name.clone().unwrap_or_default(),
                    g.type_hash.unwrap_or(0),
                    parts.join(" ")
                );
            }
        }
    ...
    // distinct root as SHARED (must not be remapped by a clone).
    {
        ...
        let mut refs: std::collections::HashMap<u32, std::collections::HashSet<u32>> =
            std::collections::HashMap::new();
        let mut ref_by: std::collections::HashMap<u32, std::collections::HashSet<u32>> =
            std::collections::HashMap::new();
        for g in &groups {
            let (Some(schema), Some(data)) = (g.schema(), g.data.as_ref()) else {
                continue;
        ...
        // Car "(Driver)" roots.
        let mut roots: Vec<(u32, String)> = names
            .iter()
            .filter(|(_, n)| {
                n.contains("(Driver)")
                    && (n.contains("Car") || n.contains("CRX") || n.contains("Sports"))
            })
            .map(|(h, n)| (*h, n.clone()))
            .collect();
        roots.sort_by_key(|(h, _)| *h);
        println!("\n=== car '(Driver)' spawnable roots ({}) ===", roots.len());
        for (h, n) in roots.iter().take(30) {
            println!("  0x{h:08X}  {n}");
        }
        ...
        let chosen: Vec<(u32, String)> = roots
            .iter()
            .filter(|(_, n)| n.contains("CRX"))
            .cloned()
            .collect();
        for (root, rname) in chosen {
            ...
            loop {
                ...
                let cands: Vec<u32> = cluster
                    .iter()
                    .filter_map(|h| refs.get(h))
                    .flatten()
                    .copied()
                    .filter(|c| !cluster.contains(c))
                    .collect();
                for c in cands {
                    if names.get(&c).is_some_and(|n| n != &rname) {
                        continue;
                    } // other named template
                    let referrers = ref_by.get(&c);
                    let all_in = referrers
                        .map(|s| s.iter().all(|r| cluster.contains(r)))
                        .unwrap_or(true);
                    if all_in {
                        cluster.insert(c);
                        added = true;
            ...
            println!(
                "\n=== PRIVATE cluster of 0x{root:08X} \"{rname}\": {} nodes (clone these) ===",
                cluster.len()
            );
            let mut cv: Vec<u32> = cluster.iter().copied().collect();
            cv.sort();
            for hh in &cv {
                // which COMPs does this node carry?
                let comps: Vec<String> = groups
                    .iter()
                    .filter(|g| {
                        let (Some(sc), Some(d)) = (g.schema(), g.data.as_ref()) else {
                            return false;
                        };
                        if sc.is_variable_length() {
                            return false;
                        }
                        let st = sc.record_stride();
                        let Some(hd) = detect(d.len(), st) else {
                            return false;
                        };
                        d[hd..]
                            .chunks_exact(st)
                            .any(|r| u32::from_le_bytes([r[0], r[1], r[2], r[3]]) == *hh)
                    })
                    .map(|g| g.name.clone().unwrap_or_default())
                    .collect();
                println!(
                    "  0x{hh:08X} {:24} [{}]",
                    names.get(hh).cloned().unwrap_or_default(),
                    comps.join(",")
                );
            }
            println!(
                "  KEEPS {} external (shared) refs: {}",
                ext_refs.len(),
                ext_refs
                    .iter()
                    .take(24)
                    .map(|h| format!(
                        "0x{h:08X}{}",
                        names.get(h).map(|n| format!("({n})")).unwrap_or_default()
                    ))
                    .collect::<Vec<_>>()
                    .join(" ")
            );
        ...
        for g in &groups {
            ...
            let n = d[hd..]
                .chunks_exact(st)
                .filter(|r| u32::from_le_bytes([r[0], r[1], r[2], r[3]]) == src)
                .count();
            if n > 0 {
                total += n;
                let has_model = g.name.as_deref() == Some("ModelName");
                let nrec_total = (d.len() - hd) / st;
                let head_u32: Vec<String> = d[..hd.min(d.len())]
                    .chunks(4)
                    .map(|c| {
                        format!(
                            "0x{:08X}",
                    ...
                    .collect();
                println!(
                    "  {:>26} (0x{:08X}) stride {st} hdr {hd}B [{}] nrec_total {nrec_total} x{n}{}",
                    g.name.clone().unwrap_or_default(),
                    g.type_hash.unwrap_or(0),
                    head_u32.join(","),
                    if has_model {
        ...
        if let Some(g) = groups.iter().find(|g| g.name.as_deref() == Some(cname)) {
            if let (Some(sc), Some(d)) = (g.schema(), g.data.as_ref()) {
                ...
                let try_v = |v: usize| -> Option<(usize, usize)> {
                    ...
                    let mut vals = 0usize;
                    while pos + 8 <= d.len() {
                        let n = u32::from_le_bytes([d[pos], d[pos + 1], d[pos + 2], d[pos + 3]])
                            as usize;
                        let key =
                            u32::from_le_bytes([d[pos + 4], d[pos + 5], d[pos + 6], d[pos + 7]]);
                        let key_ok = (key & 0xFFFF_0000) == 0x8000_0000
                            || (key & 0xFFFF_0000) == 0x9000_0000
                            || key < 0x0010_0000;
                        if !key_ok || n > 200_000 {
                            return None;
                        }
                ...
                let try_shared = |p: usize| -> Option<(usize, usize)> {
                    ...
                    while pos + 4 <= d.len() {
                        ...
                        if !((key & 0x8000_0000) != 0 || key < 0x0010_0000) {
                            return None;
                        }
                        pos += n * 4 + p;
                        buckets += 1;
                ...
                let sizes = [
                    4usize, 6, 8, 10, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68,
                    72, 84, 100, 116, 124, 144,
                ];
                let mut mm = Vec::new();
                let mut soa = Vec::new();
                let mut shr = Vec::new();
                for &v in &sizes {
                    if let Some((k, vv)) = try_v(v) {
                        mm.push(format!("V{v}({k}k/{vv}v)"));
                    }
                }
                for &p in &sizes {
                    if let Some((g, r)) = try_soa(p) {
                        soa.push(format!("P{p}({g}g/{r}r)"));
                    }
                }
                for &p in &sizes {
                    if let Some((b, k)) = try_shared(p) {
                        shr.push(format!("P{p}({b}b/{k}k)"));
                    }
                }
                println!(
                    "  {:>20} ps={:>3} len={:>7} | MM: {} | SoA: {} | SHARED: {}",
                    cname,
                    ps,
                    d.len(),
                    if mm.is_empty() {
            ...
            if let Some(d) = g.data.as_ref() {
                ...
                for o in (0..200.min(d.len())).step_by(4) {
                    if o + 4 <= d.len() {
                        ...
                        let tag = if (w & 0xFFFF_0000) == 0x8000_0000 {
                            "handle"
                        } else if w < 0x1000 {
                            "small"
                        ...
                        println!(
                            "  +{o:>4} (0x{o:03X}): 0x{w:08X}  {:>12.4}  {tag}",
                            if tag == "~float" { f } else { 0.0 }
                        );
                    }
                }
            }
    ...
    if let Some(g) = groups.iter().find(|g| g.name.as_deref() == Some("Name")) {
        if let Some(data) = g.data.as_ref() {
            println!(
                "\n=== Name COMP data: {} bytes; schm fields: {} ===",
                data.len(),
                g.schema()
                    .map(|s| s
                        .fields
                        ...
                        .map(|f| format!(
                            "0x{:08X}:{:?}@{}",
                            f.name_hash, f.field_type, f.byte_offset
                        ))
                        .collect::<Vec<_>>()
                        .join(", "))
                    .unwrap_or_default()
    ...
    if let Some(gm) = gm_container {
        println!(
            ...
            gm.len(),
            std::str::from_utf8(&gm[0..4.min(gm.len())]).unwrap_or("????")
        );
