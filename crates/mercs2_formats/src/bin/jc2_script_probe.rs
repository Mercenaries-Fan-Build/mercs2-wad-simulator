...
fn main() {
    ...
    let blk = if path.ends_with(".bin") {
        ...
        for n in &names {
            ...
            for a in arch.aset.iter().filter(|a| a.asset_hash == h) {
                println!(
                    "  {n} (0x{h:08X}): block {} sub 0x{:04X} type 0x{:08X} sec 0x{:08X}",
                    a.block_index(),
                    a.sub_entry(),
                    a.type_id,
                    a.secondary_ref
        ...
        for bi in 0..arch.indx.len() {
            let Ok(d) = decompress_block(&mut f, &arch.indx, bi as u16) else {
                continue;
            };
            if d.len() < 4 {
                continue;
            ...
            if (0..c).any(|e| {
                4 + e * 16 + 16 <= d.len() && read_u32_le(&d, 4 + e * 16 + 4) == 0x5647_C35D
            }) {
                blk = Some(d);
                break;
            }
    ...
    if names.first().map(|s| s == "--summary").unwrap_or(false) {
        ...
        for ei in 0..count {
            ...
            *hist.entry(meta).or_default() += 1;
            n_scripts += 1;
            if meta == 0 && zero_examples.len() < 10 {
                zero_examples.push(name);
            }
        }
        println!("scanned {n_scripts} script entries");
        println!("metadata-byte histogram: {hist:?}");
        println!(
            "scripts with metadata==0 ({}): {:?}",
    ...
    let want: Vec<(String, u32)> = names
        .iter()
        .map(|n| (n.clone(), pandemic_hash_m2(n)))
        .collect();
    ...
    for ei in 0..count {
        ...
        let c = &blk[pos..pos + sz];
        pos += sz;
        let Some((name, _)) = want.iter().find(|(_, h)| *h == nh) else {
            continue;
        };
            ...
            if u0 != 0xFFFF_FFFF {
                ...
                if tag == "DEPS" {
                    ...
                    if !body.is_empty() {
                        ...
                        for k in 0..dn {
                            if 1 + k * 4 + 4 <= body.len() {
                                print!("0x{:08X} ", read_u32_le(body, 1 + k * 4));
                            }
                        }
                    }
                }
        ...
        if c.len() >= 8 && &c[c.len() - 8..c.len() - 4] == b"CSUM" {
            ...
            println!(
                "  CSUM: stored 0x{stored:08X} computed 0x{computed:08X} {}",
                if stored == computed {
                    "✓"
                } else {
                    "✗ MISMATCH"
