    ...
    for &h in &hashes {
        ...
        for block in blocks {
            let dec = match decompress_block(&mut file, &archive.indx, block) {
                Ok(d) => d,
                Err(err) => {
                    println!("  block {block}: decompress err {err}");
                    continue;
                }
            };
            ...
            for e in &entries {
                ...
                if is_tex
                    || (e.type_hash == TYPE_HASH_TEXTURE
                        && entries.iter().filter(|x| x.name_hash == h).count() == 0)
                {
                    println!(
...
fn dump_info(container: &[u8]) {
    ...
    for i in 0..n_desc {
        ...
        let end = start + sz;
        println!(
            "      row {tags} u0={u0} sz={sz} f3=0x{f3:08X} f4=0x{f4:08X} span[{start}..{end}]"
        );
        if tag == b"INFO" && end <= container.len() {
            ...
            if info.len() >= 18 {
                println!(
                    "        -> w={} h={} f4field={} mip@6={} fourcc={:?}",
                    read_u16_le(info, 0),
                    read_u16_le(info, 2),
                    read_u16_le(info, 4),
                    read_u16_le(info, 6),
                    std::str::from_utf8(&info[14..18]).unwrap_or("?")
                );
            }
        }
    }
